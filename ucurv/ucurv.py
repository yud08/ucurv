...
def tan_theta_grid(S1, S2, engine: str = "auto"):
    """
...
def fftflip(F, dirlist = None, engine: str = "auto"):
    """
...
def angle_fun(Mgrid, n, alpha, dir, bandpass = None, engine: str = "auto"):
    """
    ...
    for id in range(math.ceil(n/2)):
        ang2 = -1 + id*angd + ang
        x = fun_meyer(Mgrid, ang2, engine = engine)
        if bandpass is not None:
            x = x*bandpass
        x = ncp.roll(x, 3*sp//4, (0,1) )
        Mang[id] = x.copy()
        Mang[n-1-id] = fftflip(x, dir, engine = engine)
        
    return Mang

def angle_kron(F, dr, sz, engine: str = "auto"):
    """
...
def downsamp(band, samp, shift = None, engine: str = "auto"):
    """
...
def upsamp(band, samp, shift = None, engine: str = "auto"):
    """
...
def ucurvfwd(img, udct): #will return either on the CPU or GPU depending on what engine is being used
    ...
    if udct.complex:
        ...
        imband[(0,)] = downsamp(bandfilt, Sampling[(0)], engine = engine)
        for id, subwin in Msubwin.items():
            if udct.sparse:
            ...
            imband[id] = downsamp(bandfilt, Sampling[(id[0], id[1])], engine = engine)
            id2 = list(id)
            id2[1] = id2[1] + udct.dim
            bandfilt = ncp.sqrt(0.5)*ncp.fft.ifftn(imf *fftflip(subwin))
            imband[tuple(id2)] = downsamp(bandfilt, Sampling[(id[0], id[1])], engine = engine)

    else:    
        bandfilt = ncp.real(ncp.fft.ifftn(imf*FL))
        imband[(0,)] = downsamp(bandfilt, Sampling[(0)], engine = engine) # ncp.real(ncp.fft.ifftn(imf*FL))
        for id, subwin in Msubwin.items():
            if udct.sparse:
                sbwin = ncp.zeros(udct.sz)
                sbwin[subwin[0]] = subwin[1]
...
def ucurvinv(imband, udct):
    ...
    Sampling = udct.Sampling
    # imlow = imband[0]
    imlow = upsamp(imband[(0,)], Sampling[(0)], engine = engine)
    ...
    for id, subwin in Msubwin.items():
        ...
        if udct.complex:
            bandup = upsamp(imband[id], Sampling[(id[0], id[1])], engine = engine)
            recon = recon + ncp.sqrt(0.5)*ncp.fft.ifftn( ncp.fft.fftn(bandup) * subwin )
            id2 = list(id)
            id2[1] = id2[1] + udct.dim
            bandup = upsamp(imband[tuple(id2)], Sampling[(id[0], id[1])], engine = engine)
            recon = recon + ncp.sqrt(0.5)*ncp.fft.ifftn( ncp.fft.fftn(bandup) * fftflip(subwin, engine = engine) )
        else:
            bandup = upsamp(imband[id], Sampling[(id[0], id[1])], engine = engine)
    ...
    if udct.high == 'wavelet':
        ...
        recon = meyerinvmd(band, engine = engine)
